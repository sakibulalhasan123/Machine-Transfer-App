...
//   const validateForm = () => {
//     const newErrors = {};
//     if (!formData.factoryId) newErrors.factoryId = "Factory Name is required";
//     if (!formData.machineCategory)
//       newErrors.machineCategory = "Machine Category is required";
//     if (!formData.machineGroup)
//       newErrors.machineGroup = "Machine Group is required";
...
//           Swal.fire("Error", "❌ Workbook is empty", "error");
//           return;
//         }
...
function AddMachine() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // prepare body (purchaseDate -> null if empty)
      const body = {
        factoryId: formData.factoryId,
        machineCategory: formData.machineCategory,
        machineGroup: formData.machineGroup,
  ...
  // ---------------- Bulk Add ----------------
  const handleDownloadTemplate = () => {
    // sample has purchaseDate column optional (leave empty)
    const sampleData = factories.slice(0, 2).map((f, i) => ({
      factoryName: f.factoryName || `Factory ${i + 1}`,
      machineCategory: "CNC",
      machineGroup: "Lathe",
      machineCode: `M00${i + 1}`,
      purchaseDate: "", // optional column (user can fill in Excel) - left blank here
    }));
  ...
  // helper: normalize cell value to yyyy-mm-dd for <input type="date">
  // const toDateInputString = (val) => {
  //   if (!val && val !== 0) return "";
  //   // if already yyyy-mm-dd string, try to keep
  //   if (typeof val === "string") {
  //     const maybe = new Date(val);
  //     if (!isNaN(maybe)) return maybe.toISOString().slice(0, 10);
  //     // if it's like Excel date number (rare here), attempt Number -> Date below
  //   }
  //   if (typeof val === "number") {
  //     // Excel date -> JS date conversion: (days since 1899-12-31)
  //     // XLSX usually returns JS date object if cell is date, but keep fallback:
  //     const date = XLSX.SSF
  ...
  const handleBulkUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const ws = wb.Sheets[wb.SheetNames[0]];
        const jsonData = XLSX.utils.sheet_to_json(ws, { defval: "" }); // defval: "" to keep empty cells

        const mappedData = jsonData.map((row, index) => {
          const factory = factories.find(
            (f) =>
              String(f.factoryName || "")
                .toLowerCase()
                .trim() ===
          ...
          const purchaseDate = toDateInputString(purchaseDateRaw);
          ...
          return {
            ...
            machineGroup: row.machineGroup || "",
            machineCode: row.machineCode || "",
            purchaseDate: purchaseDate, // '' or 'yyyy-mm-dd'
          };
        });
        ...
        // Excel duplicates (within file)
        const codeCounts = {};
        mappedData.forEach((row) => {
          const c = row.machineCode || "__EMPTY__";
          if (!codeCounts[c]) codeCounts[c] = 0;
          codeCounts[c]++;
        });
        const excelDuplicates = mappedData
          .filter((row) => codeCounts[row.machineCode] > 1)
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      // include purchaseDate (null if empty string)
      const payload = bulkData.map(
        ({
          factoryId,
          machineCategory,
  ...
  return (
    <>
      <Navbar />
      <div className="flex items-center justify-center min-h-screen bg-gray-50 px-6 py-8">
        <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-5xl border border-gray-200">
          <div className="flex items-center border-b pb-4 mb-6">
            <FaIndustry className="text-gray-700 text-2xl mr-2" />
            <h2 className="text-xl font-semibold text-gray-800">
              Add Machines
          ...
          {/* Bulk Upload */}
          {activeTab === "bulk" && (
            <div className="flex flex-col items-center justify-center py-6 border-2 border-dashed border-gray-300 rounded-lg space-y-4">
              <p className="text-gray-600">
                Upload an Excel file (.xlsx) with machines (purchaseDate
                optional)
              </p>
              <div className="flex gap-4">
                <button
